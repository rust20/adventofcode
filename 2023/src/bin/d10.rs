...
fn part1(inp: &str) {
    ...
    let visit = |stack: &mut VecDeque<_>, memo: &mut Vec<Vec<_>>, x, y, val| {
    ...
    while !stack.is_empty() {
        let (x, y): (i32, i32) = stack.pop_front().unwrap();
        if 0 > x || x >= map_w as i32 || 0 > y || y >= map_h as i32 {
            continue;
        }
...
fn part2(inp: &str) {
    ...
    let visit = |stack: &mut VecDeque<_>, memo: &mut Vec<Vec<_>>, x, y, _val| {
        if 0 > x || x >= map_w as i32 || 0 > y || y >= map_h as i32 {
            return;
        }
